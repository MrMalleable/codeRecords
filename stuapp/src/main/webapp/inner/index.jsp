	...
	<script type="text/javascript">
		function load(){
		    ...
		    if("success_update"==state){
		        history.pushState(history.state,null,"/inner/index.jsp");
			}
		}
	</script>
...
<body onload="load()">
	...
	<div id="container">
		<div id="top">
			<div class="t_left" onclick="location.href='index.jsp'">
				<div id="logo"></div>
			...
			<div class="t_right">
					<div class="admin">
						<ul>
							<li class="drop-down">
								<a><img src="../images/adm.png" alt=""><p><%=((StudentType)request.getAttribute("studentType")).getUsername()%></p></a>
		...
		<div id="main">
			<div class="system" onclick="location.href='http://localhost:8080/displaygrade?username=<%=((StudentType)request.getAttribute("studentType")).getUsername()%>'">
				<div id="img_part"><img src="../images/find.png" alt="学生管理系统"></div>
				<div id="enter"><h1>成绩查询</h1><p id="grade">查询个学期、各学年成绩</p></div>
			</div>
			<div class="system" onclick="location.href='http://localhost:8080/displayclass?username=<%=((StudentType)request.getAttribute("studentType")).getUsername()%>'">
				<div id="img_part"><img src="../images/classes.png" alt="学生管理系统"></div>
					<div id="enter"><h1>选课系统</h1><p id="grade">学员根据兴趣自由选择</p></div>
				</div>
		</div>
	...
	<!-- 模态框（Modal） -->
	<!--提交一个表单-->
	<form class="form-inline" action="http://localhost:8080/modifyinfo?location=index" onsubmit="return validate_form(this)" method="post">
		<div class="modal fade v-align" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="modal-body">
						<div id="info_form">
								<div class="form-group" style="margin: 15px 15px 15px 0">
									...
									<input type="text" name="username" class="form-control" id="username" value="<%=((StudentType)request.getAttribute("studentType")).getUsername()%>" readonly>
								</div>
								<div class="form-group" style="margin: 15px 15px 15px 0">
									<label for="stuid">学　号:</label>
									<input type="text" name="stuid" class="form-control" id="stuid" value="<%=((StudentType)request.getAttribute("studentType")).getStuid()%>" readonly>
								</div>
								<div class="form-group" style="margin: 15px 15px 15px 0">
									<!--删除label标签的for属性-->
									<label>性　别：</label>
									<select name="sex" class="form-control">
										<c:choose>
											<c:when test='<%=((StudentType)request.getAttribute("studentType")).getSex().equals("男")%>'>
								...
								<div class="form-group" style="margin: 15px 15px 15px 0">
									...
									<input type="text" name="academy" class="form-control" id="academy" value="<%=((StudentType)request.getAttribute("studentType")).getAcademy()%>" readonly>
								</div>
								<div class="form-group" style="margin: 15px 15px 15px 0">
									<label for="class_in">所属班级:</label>
									<input type="text" name="class_in" class="form-control" id="class_in" value="<%=((StudentType)request.getAttribute("studentType")).getClass_in()%>" readonly>
								</div>
								<div class="form-group" style="margin: 15px 15px 15px 0">
