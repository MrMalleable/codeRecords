	...
	<script type="text/javascript">
        function load(){
            ...
            if("success_update"==state){
                history.pushState(history.state,null,"/inner/details_of_class.jsp");
            }
        }
	</script>
...
<body onload="load()">
    ...
    <div id="container">
		...
		<div id="main">
			...
			<div id="rside"></div>
			<%--测试在当前页面是能获取到session传过来的值的
			<div id="test">测试：<%=((List)session.getAttribute("subjectTypeList")).isEmpty()%></div>
			--%>
		</div>
	</div>
	<!-- 模态框（Modal） -->
	<!--提交一个表单-->
	<!--和日期控件冲突，去掉tabindex和role的属性 tabindex="-1" role="dialog" -->
	<form class="form-inline" action='http://localhost:8080/modifyinfo?location=class&username=username=<%=((StudentType)session.getAttribute("studentType")).getUsername()%>' onsubmit="return validate_form(this)" method="post">
		...
		<script type="text/javascript">
			$(function(){
      		...
      		function load_clazz(){
           		document.getElementById("rside").innerHTML = '<object type="text/html" data="/inner/class.jsp" width="100%" height="100%"></object>';
      		}
		</script>
</body>
</html>